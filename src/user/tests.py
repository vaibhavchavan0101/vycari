...
class UserAuthenticationTestCase(TestCase):
    ...
    def test_user_authentication_with_username(self):
        response = self.client.post(
            self.url, {'username': user_data['username'], 'password': user_data['password']})
    ...
    def test_user_authentication_with_invalid_username(self):
        ...
        data['username'] = 'test'
        response = self.client.post(
            self.url, {'username': data['username'], 'password': data['password']})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_user_authentication_with_email(self):
        response = self.client.post(
            self.url, {'username': user_data['email'], 'password': user_data['password']})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_user_authentication_with_phone(self):
        response = self.client.post(
            self.url, {'username': user_data['phone'], 'password': user_data['password']})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_user_authentication_with_invalid_phone(self):
        ...
        response = self.client.post(
            self.url, {'username': data['phone'], 'password': data['password']})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
